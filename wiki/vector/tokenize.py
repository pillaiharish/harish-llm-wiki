"""Deterministic, dependency-free tokenizer for the vector backend (Prompt 29).

The vector backend reuses the exact same tokenization contract as the
BM25 backend (Prompt 28): the same lowercasing, the same punctuation
stripping, the same stopword list, the same ``min_length`` /
``drop_numeric`` semantics. The function is physically reimplemented
in this module to keep the import graph acyclic (``wiki.vector`` does
**not** import ``wiki.search``); the byte-level equivalence is
enforced by a unit test
(:class:`tests.test_prompt29_vector_search.TestTokenizerConsistency`).
...
6. Drop pure-numeric tokens (configurable; default keeps them so
   queries like ``"vllm 0.6"`` still match).

The stopword list is bundled at
``wiki/vector/data/stopwords_en.txt`` and is required to be
byte-identical to the BM25 list at
``wiki/search/data/stopwords_en.txt``.
"""
...
#: lowercase, no duplicates. The file is intentionally a copy of the
#: BM25 stopword list; see :class:`TestTokenizerConsistency`.
_STOPWORDS_PATH = (
...
def _load_stopwords() -> FrozenSet[str]:
    ...
    The function reads ``wiki/vector/data/stopwords_en.txt`` once
